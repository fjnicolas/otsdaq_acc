...
void FEACCInterface::configure(void)
{
        ...
        for(ACDC& acdc : acdcs)
	{
	    ...
	    OtsUDPFirmwareCore::writeAdvanced(writeBuffer, /*address*/ 0x100, /*data*/ 0x00D00000 | (1 << (acdc.getBoardIndex() + 24)));
	    OtsUDPHardware::write(writeBuffer);
	
            //wait until we have fully received all 32 expected words from the ACDC
            int iTimeout = 10;
	    OtsUDPFirmwareCore::readAdvanced(writeBuffer, 0x1138+acdc.getBoardIndex());  // This register of ACC stores the size of the buffer which stores words from ACDC.
		...
		writeBuffer,
		0x1200+acdc.getBoardIndex(), 32, 0x08, true);//NO_ADDR_INC=0x08, clear_buffer=true
	    
	    OtsUDPHardware::read(writeBuffer, readBuffer);

            std::vector<uint64_t> acdcInfo(readBuffer.size()/8);
	    memcpy((void*)acdcInfo.data(), (void*)readBuffer.c_str(), readBuffer.size());
		...
		OtsUDPFirmwareCore::readAdvanced(
		    writeBuffer,
		    0x1138+acdc.getBoardIndex());  // This register of ACC stores the size of the buffer which stores words from ACDC.
		...
		OtsUDPHardware::read(writeBuffer, readBuffer);
		std::vector<uint64_t> acdcInfo(readBuffer.size()/8);
		memcpy((void*)acdcInfo.data(), (void*)readBuffer.c_str(), writeBuffer.size());
	    ...
	    __CFG_COUT__ << "Done configuring an ACDC board." << std::endl;
        }
...
void FEACCInterface::start(std::string runNumber)
{
	...
	// Run Start Sequence Commands TODO
...
void FEACCInterface::stop(void)
{
	...
	// Run Stop Sequence Commands TODO
...
int FEACCInterface::createAcdcs()
{
	...
	if(connectedBoards.size() == 0)
	{
		...
		std::string writeBuffer;
		OtsUDPFirmwareCore::writeAdvanced(writeBuffer, 0x000, 0xFFFF0000);
		OtsUDPHardware::write(writeBuffer);
		usleep(10000);
...
void FEACCInterface::scanLinkPhase(unsigned int boardMask, bool print)
{
    ...
    for(int iOffset = 0; iOffset < 24; ++iOffset)
    {
		...
		std::string readBuffer;
		OtsUDPFirmwareCore::readAdvanced(writeBuffer,
		    0x1120, 8, 0, true);//flags=0, clear_buffer=true
		OtsUDPHardware::read(writeBuffer, readBuffer);
		std::vector<uint64_t> decode_errors(readBuffer.size()/8);
		memcpy((void*)decode_errors.data(), (void*)readBuffer.c_str(), readBuffer.size());
        if(print)
        {

	    OtsUDPFirmwareCore::readAdvanced(writeBuffer,
					   0x1110, 8, 0, true);//flags=0, clear_buffer=true
	    OtsUDPHardware::read(writeBuffer, readBuffer);
	    std::vector<uint64_t> prbs_errors(readBuffer.size()/8);
	    memcpy((void*)prbs_errors.data(), (void*)readBuffer.c_str(), readBuffer.size());
